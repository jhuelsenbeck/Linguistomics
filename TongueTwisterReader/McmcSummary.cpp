...
void McmcSummary::output(std::ofstream& file, std::set<std::string> selectedAlignments) {
    double cutoff = 0.95;
    ...
    for (int i=0; i<alignments.size(); i++)
        ...
        std::set<std::string>::iterator it = selectedAlignments.find(a->getName());
        if (selectedAlignments.size() > 0 && it == selectedAlignments.end())
            continue;
        ...
        if (a->size() > 0)
            {
            auto cogAlns = a->toJson(cutoff, file);
    ...
    if (statePartitions != NULL && ns != 0)
        ...
        json["part_freqs"] = statePartitions->toJson(subsetFreqs, file);
        }
