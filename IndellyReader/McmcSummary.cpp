...
void McmcSummary::output(UserSettings& settings) {
    auto& nytril = *new std::ofstream(settings.getPath() + "/summary.nytril", std::ios::out);
    nytril << "// This file is auto-generated by IndellyReader. Do not edit\n\n";
    ...
    for (int i=0; i<alignments.size(); i++)
        {
        nlohmann::json cogAlns = alignments[i]->toJson(0.95, nytril);
    ...
    file << j;
    std::cout << "JSON file written to: " << settings.getPath() << "summary.json" << std::endl;
}
